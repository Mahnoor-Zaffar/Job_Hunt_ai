"""Lever adapter — jobs.lever.co application forms."""
...
class LeverAdapter(BaseATSAdapter):
    ...
    def __init__(self, page: Any, apply_url: str) -> None:
        config = AdapterConfig(
            platform=ATSPlatform.LEVER,
            apply_url=apply_url,
            submit_selector="button.application-submit, .postings-btn",
            upload_resume_selector="input[type='file'][id*='resume'], input.resume-upload",
            confirm_selector=".application-confirmation, .posting-confirmation",
            field_mapping={
